...
export function isFollowUpPrompt(prompt) {
  ...
  const directReference = /^(?:وده|ودي|دي|ده|دول|هو ده|هي دي|w da|w de|w dah|w di)(?:\s|$)/i.test(text)
    || /(?:^|\s)(?:نفس الكلام|نفس الموضوع|اللي فات|الي فات|السابق|فوق|اخر رد|آخر رد|nafs el kalam|nafs elly fat|nafs eli fat|previous|above|last answer|same one|that answer|this answer|that one|this one|those|them)(?:\s|$)/i.test(text)
